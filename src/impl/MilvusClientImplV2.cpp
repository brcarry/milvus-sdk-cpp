...
json
MilvusClientImplV2::CreateImportJobs(const std::string& collection_name, const std::vector<std::string>& files,
                                     const std::string& db_name, const std::string& api_key,
                                     const std::string& partition_name, const json& options) {
    httplib::Client client(connection_->Host(), connection_->Port());

    json request_payload = {{"dbName", db_name}, {"collectionName", collection_name}, {"files", json::array({files})}};
    ...
    std::string url = "/v2/vectordb/jobs/import/create";
    ...
    auto res = client.Post(url, headers, body, "application/json");
...
json
MilvusClientImplV2::ListImportJobs(const std::string& collection_name, const std::string& db_name,
                                   const std::string& api_key) {
    httplib::Client client(connection_->Host(), connection_->Port());
    json request_payload = {{"collectionName", collection_name}, {"dbName", db_name}};

    std::string url = "/v2/vectordb/jobs/import/list";
    std::string body = request_payload.dump();
...
json
MilvusClientImplV2::GetImportJobProgress(const std::string& job_id, const std::string& db_name,
                                         const std::string& api_key) {
    httplib::Client client(connection_->Host(), connection_->Port());
    json payload = {{"dbName", db_name}, {"jobID", job_id}};

    std::string url = "/v2/vectordb/jobs/import/get_progress";
